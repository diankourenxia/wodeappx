export type WodeAppBuiltinSkinId =
  | "default"
  | "classic-blue"
  | "beauty"
  | "supor"
...
/** Built-in skins plus custom plaza skins (`plaza-*`). */
export type WodeAppSkinId = WodeAppBuiltinSkinId | `plaza-${string}`;
...
export function isWodeAppPlazaSkinId(value: string | null | undefined): value is `plaza-${string}` {
  return /^plaza-[a-z][a-z0-9-]{0,54}$/.test(String(value || ""));
}

export function isWodeAppSkinId(value: string | null | undefined): value is WodeAppSkinId {
  return isWodeAppBuiltinSkinId(value) || isWodeAppPlazaSkinId(value);
}
