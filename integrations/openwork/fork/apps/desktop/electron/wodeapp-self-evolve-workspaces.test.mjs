...
test("OSS standalone wrap mounts nested wodeappx without runtime-server", () => {
  const wrap = mkdtempSync(path.join(tmpdir(), "wodeappx-oss-wrap-"));
  writePackage(path.join(wrap, "wodeappx"), "wodeappx");
  mkdirSync(path.join(wrap, "wodeappx", "scripts"), { recursive: true });
  writeFileSync(path.join(wrap, "wodeappx", "scripts", "self-evolve-guard.mjs"), "// fixture\n");

  assert.equal(resolveExtractedSelfEvolveMount(wrap), path.resolve(path.join(wrap, "wodeappx")));

  const specs = listSelfEvolveWorkspaceSpecs({
    env: {},
    isPackaged: true,
    seedPaths: [resolveExtractedSelfEvolveMount(wrap)],
  });
  assert.equal(specs.length, 1);
  assert.equal(specs[0].name, SELF_EVOLVE_WORKSPACE_NAME);
  assert.equal(specs[0].path, path.resolve(path.join(wrap, "wodeappx")));
});
