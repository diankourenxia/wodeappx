 ...
 * Returns the mount path (full monorepo `…/wodeapp`, or nested `…/wodeapp/wodeappx`
 * for standalone OSS packs) or "".
 *
 * @param {{
 ...
 *   resolveMount?: (dir: string) => string,
 *   log?: (...args: any[]) => void,
 * }} options
 */
export async function ensureBundledSelfEvolveMonorepo(options) {
  ...
  const accept = (dir) => {
    if (typeof resolveMount === "function") {
      const mount = String(resolveMount(dir) || "").trim();
      return mount;
    }
    if (looksLikeMonorepoRoot(dir)) return dir;
    const nested = path.join(dir, "wodeappx");
    if (existsSync(path.join(nested, "scripts", "self-evolve-guard.mjs"))) return nested;
    if (existsSync(path.join(dir, "scripts", "self-evolve-guard.mjs"))) return dir;
    return "";
  };
  ...
  return withDestLock(destRoot, async () => {
    ...
    if (lockedMount) return lockedMount;
    ...
    try {
      ...
      const mount = accept(monorepoRoot);
      if (!mount) {
