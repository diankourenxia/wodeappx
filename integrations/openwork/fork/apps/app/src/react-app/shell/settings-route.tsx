...
import "@/react-app/domains/settings/wodeapp-mcp-config";
import "@/react-app/domains/settings/browseract-mcp-config";
import "@/react-app/domains/settings/feishu-mcp-config";
...
import { WodeAppServiceView } from "@/react-app/domains/wodeapp/wodeapp-service-view";
...
function parseSettingsPath(pathname: string): {
  ...
  const trimmed = pathname
    ...
    .replace(/^\/+|\/+$/g, "");
  if (!trimmed) {
    return { tab: "service", redirectPath: "service" };
  }

  const [head, tail] = trimmed.split("/");
  switch (head) {
    case "general":
      // Hub page removed from nav; land on the primary settings tab.
      return { tab: "service", redirectPath: "service" };
    case "ai":
    case "preferences":
    case "environment":
      // Model prefs / BYOK provider / env pages removed from product nav.
      return { tab: "service", redirectPath: "service" };
    case "service":
    case "permissions":
    case "shell":
...
function readNavigationMcpSearch(state: unknown): string | null {
  if (!state || typeof state !== "object") return null;
  const value = (state as { mcpSearch?: unknown }).mcpSearch;
  return typeof value === "string" ? value.trim() || null : null;
}

function readNavigationMcpDetailServerName(state: unknown): string | null {
  if (!state || typeof state !== "object") return null;
  const value = (state as { mcpDetailServerName?: unknown }).mcpDetailServerName;
  return typeof value === "string" ? value.trim() || null : null;
}

function readNavigationMcpConnectServerName(state: unknown): string | null {
  if (!state || typeof state !== "object") return null;
  const value = (state as { mcpConnectServerName?: unknown }).mcpConnectServerName;
  return typeof value === "string" ? value.trim() || null : null;
}
...
export type SettingsSurfaceProps = {
  embedded?: boolean;
  /** Render only the active settings view (no SettingsShell chrome). */
  contentOnly?: boolean;
  initialPath?: string;
  workspaceId?: string;
  onClose?: () => void;
...
function SettingsRouteContent(props: SettingsSurfaceProps = {}) {
  ...
  const [embeddedPath, setEmbeddedPath] = useState(props.initialPath ?? "service");
  ...
  const navigationMcpSearch = readNavigationMcpSearch(location.state);
  const navigationMcpDetailServerName = readNavigationMcpDetailServerName(location.state);
  const navigationMcpConnectServerName = readNavigationMcpConnectServerName(location.state);
  ...
  const [appIconState, setAppIconState] = useState<DesktopAppIconState | null>(null);
  ...
  // WodeAppX：彻底关闭 OpenWork Models 订阅/连接入口（含 banner 与 dismiss 后的 connect 行）。
  const openWorkModelsUiDisabled = isOpenWorkModelsPromoHidden();
  const showOpenWorkModelsSubscribe =
    !openWorkModelsUiDisabled && !openWorkModelsConnected && !openWorkModelsPromoHidden;
  ...
  const installOpenAiImageExtension = useCallback(async (apiKey: string) => {
    ...
    if (!openworkClient) {
      setImageExtensionError("WodeAppX server is not connected.");
      return;
    }
    if (!resolvedApiKey) {
    ...
    try {
      ...
      setImageExtensionStatus("Saved OPENAI_API_KEY. Agents can use WodeAppX extension actions for image generation.");
    } catch (error) {
  ...
  const generateOpenAiTestImage = useCallback(async (input: { apiKey: string; prompt: string }) => {
    ...
    if (!client || !workspaceId) {
      setImageGenerationError("WodeAppX server is not connected for this workspace.");
      return;
    }
    if (!apiKey) {
  ...
  const testVoiceSession = useCallback(async () => {
    if (!openworkClient) {
      setVoiceError("WodeAppX server is not connected.");
      return;
    }
    setVoiceBusy(true);
    setVoiceStatus(null);
    ...
    try {
      ...
      setVoiceStatus(`Realtime ready with ${session.model} (${session.tools.length} WodeAppX tools).`);
    } catch (error) {
      setVoiceError(describeRouteError(error));
    } finally {
      setVoiceBusy(false);
  ...
  const installLocalProvider = useCallback(async (input: LocalProviderInstallInput) => {
    ...
    if (!client || !workspaceId) {
      setLocalProviderError("WodeAppX server is not connected for this workspace.");
      return;
    }
    if (!modelId) {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    void desktopBridge.applicationIconGet()
      .then((state) => {
        if (!cancelled) setAppIconState(state);
      })
      .catch((error) => {
        if (!cancelled) setAppIconError(describeRouteError(error));
      });
    return () => {
      cancelled = true;
    };
  ...
  const chooseAppIcon = useCallback(async () => {
    if (!isDesktopRuntime()) return;
    setAppIconBusy(true);
    setAppIconError(null);
    try {
      setAppIconState(await desktopBridge.applicationIconChoose());
    } catch (error) {
      setAppIconError(describeRouteError(error));
    } finally {
      setAppIconBusy(false);
    }
  }, []);

  const resetAppIcon = useCallback(async () => {
    if (!isDesktopRuntime()) return;
    setAppIconBusy(true);
    setAppIconError(null);
    try {
      setAppIconState(await desktopBridge.applicationIconReset());
    } catch (error) {
      setAppIconError(describeRouteError(error));
    } finally {
      setAppIconBusy(false);
    ...
    try {
      await openworkClient.patchConfig(workspaceId, {
        opencode: {
          compaction: {
            auto: next,
            // Keep in sync with MANAGED_COMPACTION_POLICY in
            // fork/apps/desktop/electron/wodeapp-managed-models.mjs so a shallow
            // toggle does not wipe the managed long-session soft wall.
            prune: true,
            tail_turns: 4,
  ...
  const connectComputerUseAction = useMemo<OpenworkControlAction>(() => ({
    id: "computer_use.connect",
    label: "Connect Computer Use",
    description: "Enable the built-in Computer Use extension and connect its local MCP so agents can use screenshots and desktop app controls.",
    sideEffect: "mutation",
    disabled: !isDesktopRuntime() || !isMacPlatform(),
    execute: async (_args, helpers) => {
      if (!isDesktopRuntime() || !isMacPlatform()) {
        return { ok: false, error: "Computer Use is only available in the macOS desktop app." };
      }
      const entry = connectionsStore.quickConnect.find((candidate) =>
        (candidate.id ?? candidate.serverName) === "computer-use" || candidate.serverName === "computer-use"
      );
      if (!entry) return { ok: false, error: "Computer Use extension is unavailable." };

      setOpenWorkExtensionEnabled(entry, true);
      helpers.setNarration("Connecting Computer Use MCP...");

      const alreadyConnected = connectionsSnapshot.mcpServers.some((server) => server.name === "computer-use");
      const connected = alreadyConnected || await connectionsStore.connectMcp(entry);
      await connectionsStore.refreshMcpServers();

      let permissions = computerUsePermissions;
      try {
        const nextPermissions = normalizeComputerUsePermissions(await desktopBridge.checkComputerUsePermissions());
        if (nextPermissions) {
          permissions = nextPermissions;
          setComputerUsePermissions(nextPermissions);
        }
      } catch {
        // The connection is still useful; the permission helper surfaces details in Settings.
      }
      ...
      return {
        ...
        alreadyConnected,
        permissions,
        permissionsReady: permissions?.accessibility === true && permissions.screenRecording === true,
      };
    },
  }), [computerUsePermissions, connectionsSnapshot.mcpServers, connectionsStore]);
  useControlAction(connectComputerUseAction);
  ...
  const handleSaveRenameWorkspace = useCallback(async () => {
    ...
    try {
      if (!openworkClient) {
        toast.error("WodeAppX server is unavailable. Reconnect the server before renaming workspaces.");
        return;
      }
  ...
  const handleExportWorkspaceConfig = useCallback(async (workspaceId: string) => {
    ...
    throw new Error("WodeAppX server is unavailable. Reconnect the server before exporting workspace config.");
  ...
  const handleCreateWorkspace = async (preset: WorkspacePreset, folder: string | null) => {
    ...
    try {
      ...
      if (!list) {
        throw new Error("WodeAppX server is unavailable. Start or reconnect the server before creating a workspace.");
      }
  ...
  const handleCreateRemoteWorkspace = async (input: {
    ...
    try {
      ...
      if (!list) {
        throw new Error("WodeAppX server is unavailable. Start or reconnect the server before connecting a remote workspace.");
      }
      const createdId = resolveWorkspaceListSelectedId(list) || list.workspaces[list.workspaces.length - 1]?.id || "";
      if (createdId) {
        await workspaceSetSelected(createdId).catch(() => undefined);
          ...
          <ExtensionsView
            ...
            mcpView={
              <McpView
                ...
                initialSearch={navigationMcpSearch}
                initialDetailServerName={navigationMcpDetailServerName}
                initialConnectServerName={navigationMcpConnectServerName}
                showHeader={false}
              />
            }
      ...
      case "appearance":
        return (
          <AppearanceView
            ...
            appIconBusy={appIconBusy}
            appIconError={appIconError}
            chooseAppIcon={chooseAppIcon}
  ...
  const settingsBody = props.contentOnly ? (
    <div className="wapp-settings-content-only flex min-h-0 w-full flex-1 flex-col overflow-hidden">
      {settingsView}
    </div>
  ) : (
    <SettingsShell
    ...
    <>
      ...
      <ProviderAuthModal
        ...
        onSubmitApiKey={async (providerId, apiKey) => {
          ...
          return providerAuthStore.submitProviderApiKey(resolved.providerId, apiKey);
        }}
