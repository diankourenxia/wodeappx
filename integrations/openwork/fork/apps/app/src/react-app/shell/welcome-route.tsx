...
import { isWodeAppAuthAvailable, signInWithWodeApp, type WodeAppAuthConfig } from "../../app/lib/wodeapp-auth";
import { WodeAppCloudRegionDialog } from "../domains/wodeapp/wodeapp-cloud-region-dialog";
import {
  originForCloudRegion,
  writeStoredCloudRegion,
  type WodeAppCloudRegion,
} from "../domains/wodeapp/wodeapp-cloud-region";
import { wodeAppAuthConfigToModelRef } from "../domains/wodeapp/wodeapp-model-sync";
...
export function WelcomeRoute() {
  ...
  const [wodeAppBusy, setWodeAppBusy] = useState(false);
  ...
  useEffect(() => {
    if (!shellConfig.welcomePage || local.prefs.hasCompletedOnboarding) {
      navigate("/session", { replace: true });
    }
  }, [local.prefs.hasCompletedOnboarding, navigate, shellConfig.welcomePage]);
  ...
  const applyWodeAppSession = useCallback((config: WodeAppAuthConfig) => {
  ...
  const completeWodeAppSignIn = useCallback(async (region: WodeAppCloudRegion) => {
    ...
    try {
      ...
      applyWodeAppSession(result.config);
    } finally {
    ...
    <>
      ...
      {state.providerStep ? (
        <ProviderSelectionStep
          onWodeAppSignIn={isDesktopRuntime() && isWodeAppAuthAvailable() ? handleWodeAppSignIn : undefined}
