...
export type ComposerStateStore = {
  ...
  updateQueuedDraftText: (sessionId: string, index: number, text: string) => void;
...
export const useComposerStateStore = create<ComposerStateStore>((set) => ({
  ...
  updateQueuedDraftText: (sessionId, index, text) => set((state) => {
    const current = state.queuedDrafts[sessionId];
    const existing = current?.[index];
    if (!current || !existing) return state;
    if (existing.text === text && (existing.resolvedText ?? existing.text) === text) return state;
    const nextDraft: ComposerDraft = {
      ...existing,
      text,
      resolvedText: text,
      // Editing collapses structured parts back to plain text; attachments stay.
