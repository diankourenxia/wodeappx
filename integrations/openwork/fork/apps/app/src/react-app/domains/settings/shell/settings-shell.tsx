  ...
  return (
    <div className="flex h-dvh min-h-screen w-full overflow-hidden">
      <SidebarProvider
        ...
        style={{ "--sidebar-width": "20rem" } as React.CSSProperties & { "--sidebar-width": string }}
      >
        <SettingsSidebar
          activeTab={props.activeTab}
          onSelectTab={props.onSelectTab}
...
function SettingsSectionMenu(props: Pick<SettingsPageFrameProps, "activeTab" | "developerMode" | "onSelectTab">) {
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger
        render={(
          <Button variant="outline" size="default" className="h-10 min-w-0 max-w-64 justify-start gap-2.5 px-3 text-[15px]">
      ...
      <DropdownMenuContent className="w-64">
        {sections.map((section, index) => (
          <DropdownMenuGroup key={section.label ?? "primary"}>
