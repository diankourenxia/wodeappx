...
export function WodeAppAssetTileContent({ item }: { item: DigitalAssetItem }) {
  ...
  if (item.preview === "brand") {
    const colors = (item.brandColors?.length ? item.brandColors : ["#FF6600", "#1A1A1A", "#FFFFFF"])
      .filter((color) => color.toUpperCase() !== "#FFFFFF" && color.toUpperCase() !== "#F7F7F5")
      .slice(0, 3);
    return (
      <span className="wx-asset-brand-tile" aria-hidden>
        {item.coverImage ? (
          <img className="wx-asset-brand-logo" src={item.coverImage} alt="" loading="lazy" decoding="async" />
        ) : (
