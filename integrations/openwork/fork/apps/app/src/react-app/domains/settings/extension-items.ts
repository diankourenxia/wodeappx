...
export function buildExtensionItems(input: ExtensionItemBuildInput) {
  ...
  const builtInQuickConnectEntries = builtInItems.flatMap((item) => {
    if (!item.builtInEntry) return [];
    if (item.active || item.resources.some((resource) => resource.type === "mcp")) {
      return [item.builtInEntry];
    }
    return [];
  });
