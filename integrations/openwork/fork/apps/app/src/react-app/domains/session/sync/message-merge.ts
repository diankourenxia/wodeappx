...
export function mergeSnapshotAndLiveMessages(
  ...
  if (options.appendLiveOnlyMessages) {
    for (const liveMessage of liveMessages) {
      if (snapshotIds.has(liveMessage.id)) continue;
      // Live-only turns must not reorder ahead of snapshot-resident messages
      // just because a stale live cache lists an older assistant after the new user.
      // Prefer timestamps when present; otherwise append after the snapshot floor.
      const created = messageCreated(liveMessage);
      if (created !== null) {
        insertMessageByChronology(merged, liveMessage, liveMessages);
      } else {
