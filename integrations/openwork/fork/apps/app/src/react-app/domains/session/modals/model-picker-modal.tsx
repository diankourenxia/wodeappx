...
import { normalizeWodeAppModelRef } from "../../wodeapp/wodeapp-model-display";
import {
  modelCapabilityLabels,
  modelCapabilitySearchText,
} from "../../wodeapp/wodeapp-model-capabilities";
...
export function ModelPickerModal(props: ModelPickerModalProps) {
  ...
  const filteredOptions = useMemo(() => {
    ...
    return props.options.filter(
      (o) =>
        ...
        modelCapabilitySearchText(o.capabilities).includes(q),
    );
  ...
  const handleSelect = useCallback(
    (opt: ModelOption) => props.onSelect(normalizeWodeAppModelRef({ providerID: opt.providerID, modelID: opt.modelID })),
    [props.onSelect],
  );
...
function DefaultModelRow({
  ...
  const capabilityLabels = modelCapabilityLabels(opt.capabilities);
      ...
      <div className="min-w-0 flex-1">
        ...
        <span className="mt-1 flex max-w-full flex-wrap gap-1">
          {capabilityLabels.map((label) => (
            <span
              key={label}
              className="rounded border border-dls-border px-1.5 py-0.5 text-[10px] leading-none text-dls-secondary"
            >
              {label}
            </span>
          ))}
