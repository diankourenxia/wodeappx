...
import {
  WodeAppNewConversationButton,
  WodeAppRuntimeProjectsSection,
} from "../../wodeapp/wodeapp-sidebar-sections";
import { FileTreeSection } from "../file-tree/file-tree-section";
...
function SessionMenuContent({ variant, sessionId, workspaceId, isPinned, isArchived }: SessionMenuContentProps) {
  ...
  if (variant === "dropdown") {
    return (
      <>
        ...
        {ctx.onCompactSession ? (
          <DropdownMenuItem onClick={() => void ctx.onCompactSession?.(sessionId)}>
  ...
  return (
    <>
      ...
      {ctx.onCompactSession ? (
        <ContextMenuItem onClick={() => void ctx.onCompactSession?.(sessionId)}>
          <Minimize2 className="size-4" />
          压缩上下文
        </ContextMenuItem>
      ) : null}
...
export function SessionActions({ className, sessionId, workspaceId, isPinned, isArchived }: SessionActionsProps) {
...
export function SessionContextMenu({ children, sessionId, workspaceId, isPinned, isArchived }: SessionContextMenuProps) {
...
export type AppSidebarProps = {
  ...
  onCreateTaskWithPrompt?: (workspaceId: string, prompt: string | import("@/react-app/domains/wodeapp/wodeapp-composer-handoff").WodeAppTaskPromptInput) => void | Promise<void | string | null>;
  ...
  return (
    <SidebarContext.Provider value={contextValue}>
      <Sidebar
        ...
        className="**:data-[sidebar=sidebar]:bg-[#f7f8fb] **:data-[sidebar=sidebar]:text-slate-900"
      >
        ...
        <SidebarMenu>
          ...
          >
            ...
            <CollapsibleContent className="pt-px">
              <SidebarMenuSub>
                <WodeAppNewConversationButton workspaceId={workspace.id} />
                <WodeAppRuntimeProjectsSection workspaceId={workspace.id} />
                <FileTreeSection
                  workspaceId={workspace.id}
                  workspaceRoot={workspace.path}
                  client={ctx.openworkServerClient}
                  activeSessionId={ctx.selectedWorkspaceId === workspace.id ? ctx.selectedSessionId : null}
                />
