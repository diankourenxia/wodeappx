...
type EditorProps = {
  ...
  onSubmit: (options: { steer: boolean }) => void | Promise<void>;
...
const INLINE_TOKEN_BASE_CLASS =
  "inline-flex h-9 max-w-full items-center rounded-full border text-[13px] font-semibold leading-none align-middle transition-colors";
const INLINE_TOKEN_LABEL_CLASS = "min-w-0 max-w-[190px] truncate";
const PROMPT_MENTION_LABEL_CLASS = "min-w-0 max-w-[min(420px,70vw)] truncate";
const ASSET_THUMB_CLASS =
  "inline-flex size-7 shrink-0 items-center justify-center overflow-hidden rounded-full border border-white/70 bg-violet-4/50 text-[11px] font-bold text-violet-11";
const ASSET_MEDIA_CLASS = "h-full w-full object-cover";
const COMMAND_PILL_CLASS = `${INLINE_TOKEN_BASE_CLASS} gap-1.5 border-violet-6/35 bg-violet-3/20 px-3 text-violet-11`;
const SKILL_PILL_CLASS = COMMAND_PILL_CLASS;
const PASTED_TEXT_PILL_CLASS = `${INLINE_TOKEN_BASE_CLASS} gap-1.5 border-amber-6/35 bg-amber-3/15 px-3 text-amber-11`;
const PASTED_TEXT_BUTTON_CLASS =
  "ml-0.5 inline-flex h-5 w-5 shrink-0 items-center justify-center rounded-full text-amber-10 transition-colors hover:bg-amber-4 hover:text-amber-12";
const PROMPT_MENTION_PILL_CLASS =
  `${INLINE_TOKEN_BASE_CLASS} border-violet-6/35 bg-violet-3/20 px-3 text-violet-11 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-6/40`;

const MENTION_PILL_CLASS: Record<ComposerMentionKind, string> = {
  file: `${INLINE_TOKEN_BASE_CLASS} gap-1.5 border-gray-6 bg-gray-3 px-3 text-gray-11`,
  agent: `${INLINE_TOKEN_BASE_CLASS} gap-1.5 border-sky-6/35 bg-sky-3/20 px-3 text-sky-11`,
  app: `${INLINE_TOKEN_BASE_CLASS} gap-1.5 border-cyan-6/35 bg-cyan-3/20 px-3 text-cyan-11`,
  asset: `${INLINE_TOKEN_BASE_CLASS} gap-2 border-violet-6/35 bg-violet-3/20 pl-1.5 pr-3 text-violet-11 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-6/40`,
};
...
function resolveAssetMentionMeta(value: string) {
  const asset = findDigitalAssetByMentionValue(value);
  const remembered = resolveAssetMentionById(assetIdFromMentionValue(value));
  const assetFile = asset?.assetFile || remembered?.assetFile;
  const assetFileType = asset?.assetFileType || remembered?.assetFileType || "";
  const assetKind = asset?.kind || remembered?.kind || "数字资产";
  const isBrandAsset = assetKind === "品牌库";
  const isPromptAsset = assetKind === "提示词";
  const imageUrl =
    isBrandAsset || isPromptAsset
      ? undefined
      : asset?.coverImage ||
  ...
  return {
    name: asset?.name || remembered?.name || value.replace(/^asset:/, ""),
    kind: assetKind,
    promptText: asset?.promptText || remembered?.promptText,
    imageUrl,
    videoUrl,
  };
}
...
function mentionPillText(value: string, kind: ComposerMentionKind) {
  if (kind === "asset") {
    return `@${resolveAssetMentionMeta(value).name}`;
  }
  return `@${kind === "file" ? value.split(/[\\/]/).pop() || value : value}`;
}

function dispatchAssetPreview(value: string) {
  window.dispatchEvent(new CustomEvent(PREVIEW_ASSET_MENTION_EVENT, { detail: { value } }));
}

function createInlineTokenLabel(text: string, className = INLINE_TOKEN_LABEL_CLASS) {
...
function assetMentionInitial(meta: ReturnType<typeof resolveAssetMentionMeta>) {
  return Array.from((meta.name || meta.kind || "资").trim())[0] || "资";
}

function clearElement(node: HTMLElement) {
  while (node.firstChild) node.removeChild(node.firstChild);
}

function renderAssetMentionDom(dom: HTMLElement, value: string) {
  const meta = resolveAssetMentionMeta(value);
  const isPromptAsset = meta.kind === "提示词";
  clearElement(dom);
  dom.className = isPromptAsset ? PROMPT_MENTION_PILL_CLASS : MENTION_PILL_CLASS.asset;
  dom.contentEditable = "false";
  dom.setAttribute("spellcheck", "false");
  dom.setAttribute("role", "button");
  dom.setAttribute("tabindex", "0");
  dom.title = isPromptAsset && meta.promptText
    ? `${meta.name}\n${meta.promptText}`
    : `${meta.name} · ${meta.kind}`;
  ...
  if (!isPromptAsset) {
    ...
    thumb.className = ASSET_THUMB_CLASS;
    if (meta.imageUrl) {
      ...
      image.loading = "lazy";
      image.className = ASSET_MEDIA_CLASS;
      thumb.append(image);
      ...
      video.className = ASSET_MEDIA_CLASS;
      thumb.append(video);
    } else {
      thumb.textContent = assetMentionInitial(meta);
    }
    dom.append(thumb);
  }

  const promptCopy = meta.promptText?.trim();
  dom.append(createInlineTokenLabel(
    isPromptAsset && promptCopy ? `@${meta.name}：${promptCopy}` : `@${meta.name}`,
    isPromptAsset ? PROMPT_MENTION_LABEL_CLASS : INLINE_TOKEN_LABEL_CLASS,
  ));
}

function renderMentionDom(dom: HTMLElement, value: string, kind: ComposerMentionKind) {
  if (kind === "asset") {
    renderAssetMentionDom(dom, value);
    return;
  }
  clearElement(dom);
  dom.className = MENTION_PILL_CLASS[kind];
  dom.append(createInlineTokenLabel(mentionPillText(value, kind)));
  dom.contentEditable = "false";
  dom.setAttribute("spellcheck", "false");
...
class ComposerSlashCommandNode extends TextNode {
  ...
  override createDOM(_config: EditorConfig) {
    const dom = document.createElement("span");
    dom.className = COMMAND_PILL_CLASS;
    dom.append(createInlineTokenLabel(`/${this.__commandName}`));
    dom.contentEditable = "false";
    dom.setAttribute("spellcheck", "false");
...
class ComposerSkillNode extends TextNode {
  ...
  override createDOM(_config: EditorConfig) {
    const dom = document.createElement("span");
    dom.className = SKILL_PILL_CLASS;
    dom.append(createInlineTokenLabel(this.__skillName));
    dom.contentEditable = "false";
    dom.setAttribute("spellcheck", "false");
...
function createPastedTextChipDom(label: string, lines: number) {
  const dom = document.createElement("span");
  dom.className = PASTED_TEXT_PILL_CLASS;
  dom.contentEditable = "false";
  dom.setAttribute("spellcheck", "false");
  dom.title = `Pasted text · ${label}`;

  const text = createInlineTokenLabel(pastedTextChipLabel(lines));

  const button = document.createElement("button");
  button.type = "button";
  button.className = PASTED_TEXT_BUTTON_CLASS;
...
function SubmitPlugin(props: { onSubmit: (options: { steer: boolean }) => void | Promise<void>; disabled: boolean }) {
  const [editor] = useLexicalComposerContext();
  ...
  useEffect(() => {
    return editor.registerCommand(
      ...
      (event: KeyboardEvent | null) => {
        ...
        // Plain Enter / Cmd+Enter both submit (busy = queue until idle).
        event?.preventDefault();
        void onSubmitRef.current({ steer: false });
        return true;
      },
      COMMAND_PRIORITY_HIGH,
    );
