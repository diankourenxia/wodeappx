...
import {
  ...
  cleanupPersistedManagedProcessGenerations,
  createManagedProcessGeneration,
  forgetManagedProcessGeneration,
  managedProcessSpawnOptions,
  persistManagedProcessGeneration,
  terminateManagedProcessGeneration,
  type ManagedProcessGeneration,
...
export async function createManagedOpencodeServer(options: {
  ...
  // A cleanly stopped previous server removes its record. After a crash,
  // only descendants carrying our inherited generation marker are reclaimed.
  await cleanupPersistedManagedProcessGenerations(process.env).catch(() => undefined);
  const generation = createManagedProcessGeneration(command, options.cwd);
  const child: ChildProcess = spawn(command, args, managedProcessSpawnOptions(generation, {
    cwd: options.cwd,
    env,
    stdio: ["ignore", "pipe", "pipe"],
  }));
  attachManagedProcessGeneration(generation, child.pid);
  const generationPersisted = persistManagedProcessGeneration(generation, process.env).catch(() => undefined);
  ...
  const closeGeneration = async () => {
    await generationPersisted;
    const outcome = await terminateManagedProcessGeneration(generation);
    if (outcome.terminated) {
      await forgetManagedProcessGeneration(generation, process.env).catch(() => undefined);
    }
  ...
  try {
    url = await new Promise<string>((resolve, reject) => {
      ...
      child.once("exit", (code) => {
        fail(new Error(`Local engine server exited with code ${code}${output.trim() ? `\n${output}` : ""}`));
      });
    });
  } catch (error) {
