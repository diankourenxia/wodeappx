...
export type PerfMonitorSample = {
  at: number;
  electron: {
    mainCpu: number | null;
    mainRssMiB: number | null;
    rendererCpu: number | null;
    rendererRssMiB: number | null;
    gpuCpu: number | null;
    gpuRssMiB: number | null;
    processCount: number;
  };
  engine: {
    cpu: number | null;
...
export type PerfMonitorStatus = {
  ...
  ringCount: number;
  ringMax: number;
  lastSample: PerfMonitorSample | null;
  hud: string;
};

export type PerfMonitorSnapshot = PerfMonitorStatus & {
  samples: PerfMonitorSample[];
};
...
export type ComputerUsePermissions = {
  ...
  backend?: "handsfree" | "open-computer-use" | string;
  note?: string;
  error?: string;
};
...
export type AttachmentContextPackWriteInput = {
  refId: string;
  sessionId: string;
...
export type AttachmentContextPackWriteResult = {
  ok: boolean;
  refId: string;
  contextChars: number;
  storedBytes: number;
  ...
  files: Array<{
    /** Original display name before disk-safe filename normalization. */
...
export type AttachmentContextPackStatus = {
  ok: boolean;
  packs: number;
  totalBytes: number;
  maxStoreBytes: number;
...
export type AttachmentContextPackDeleteResult = AttachmentContextPackStatus & {
  deletedPacks: number;
  freedBytes: number;
};
...
export type DesktopCommandMap = {
  ...
  perfMonitorSetEnabled: {
    args: [input: { enabled: boolean; persist?: boolean } | boolean];
    result: PerfMonitorStatus;
  };
  perfMonitorReportRenderer: {
    args: [input: { longTasks?: number; longTaskMaxMs?: number; sseEvents?: number }];
    result: { ok: boolean; pending: { longTasks: number; longTaskMaxMs: number; sseEvents: number } };
  };
  perfMonitorExport: { args: []; result: PerfMonitorExportResult };
  applicationIconGet: { args: []; result: DesktopAppIconState };
  applicationIconChoose: { args: []; result: DesktopAppIconState };
  applicationIconReset: { args: []; result: DesktopAppIconState };
  ...
  attachmentContextPut: {
    args: [input: AttachmentContextPackWriteInput];
    result: AttachmentContextPackWriteResult;
  };
  attachmentContextStatus: {
    args: [];
    result: AttachmentContextPackStatus;
  };
  attachmentContextDeleteSession: {
    args: [sessionId: string];
    result: AttachmentContextPackDeleteResult;
  };
  attachmentContextClear: {
    args: [];
    result: AttachmentContextPackDeleteResult;
  };
  ...
  /** Save a remote/local/data URL straight into Downloads — no “fetch then save” round-trip in the UI. */
  downloadUrl: {
    args: [options: { url: string; name?: string; reveal?: boolean }];
    result: { ok: boolean; path?: string; canceled?: boolean; reason?: string };
  };
  ...
  __openPath: { args: [target: string, workspaceRoot?: string]; result: unknown };
  /** Resolve a bare filename / missing path without opening it (attachment packs first). */
  __resolveOpenPath: { args: [target: string, workspaceRoot?: string]; result: string | null };
  __revealItemInDir: { args: [target: string, workspaceRoot?: string]; result: unknown };
