...
description: "End-to-end WodeApp short-drama (竖屏短剧) production pipeline. Use ONLY when the user explicitly wants a short drama / 短剧 / episode package / story bible / character bible / season outline — not for 商品短视频, 带货视频, promo clips, or '生成短视频' product ads (those use video_generate / wodeapp_video_storyboard_open). When in scope: market positioning, logline, season outline, episode hooks, full episode scripts, clip splitting, casting/style bible, scene assets, turnaround sheets, WodeApp workflow import JSON, and Seedream 5.0 asset generation/upload."
---

# WodeApp Short Drama Factory

Use this skill to turn a **short-drama / 竖屏短剧** idea into a WodeApp-ready production package, then generate selectable assets.

**Out of scope:** product short videos (商品短视频 / 带货视频 / 「用商品…生成短视频」). Those are ecommerce promo clips — use `video_generate` or `wodeapp_video_storyboard_open`, and do **not** call `wodeapp.short_drama.open`.
...
- For short-drama work, load this skill when helpful and continue from step 2 (story bible). Work in the current conversation; do not open or wait for a dedicated cloud project. Before season finalize (all episodes drafted), call AppX UI action `wodeapp.short_drama.series_preflight` with `episodes[]` and `characters`; if `blocked=true`, fix P0 issues before storyboard/video. For multi-scene video, call `wodeapp.video_storyboard.open` to inject storyboard data; the user must manually start rendering in the studio (Agent does not auto-run batch video).
