...
- For short-drama work, load this skill when helpful and continue from step 2 (story bible). Before season finalize (all episodes drafted), call AppX UI action `wodeapp.short_drama.series_preflight` with `episodes[]` and `characters`; if `blocked=true`, fix P0 issues before storyboard/video. To open the built-in 短剧项目 page for side-by-side editing, call `wodeapp.short_drama.open` — it only exposes the default page. For multi-scene video, call `wodeapp.video_storyboard.open` to inject storyboard data; the user must manually start rendering in the studio (Agent does not auto-run batch video).
