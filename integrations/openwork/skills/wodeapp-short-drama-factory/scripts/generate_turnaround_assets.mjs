...
const apiKey = process.env.WODEAPP_API_KEY || readWodeAppConfigApiKey();
if (!apiKey) fail("WODEAPP_API_KEY is required (set the env var, or sign in on WodeAppX so ~/.wodeapp/config.json has an apiKey)");
...
function readWodeAppConfigApiKey() {
  try {
    const configPath = join(homedir(), ".wodeapp", "config.json");
