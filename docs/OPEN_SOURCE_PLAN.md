...
**本轮已落地（monorepo → 独立仓）**：
- 仓已 public；正式 `v1.0.0` 四平台包（Mac arm64/x64 公证，Win unsigned，Linux AppImage）
...
|---|---|---|---|
| 独立仓库尚未验证公开可访问 | 用户无法 Fork/Issue/下载 | public + Issues + Security advisory 表单 | ✅ **public**。vulnerability reporting 已开。`main` 未开分支保护（还要 orphan force-push） |
...
**仓库已 public。** 安装包以独立仓 [`v1.0.0`](https://github.com/diankourenxia/wodeappx/releases/tag/v1.0.0) 与官网下载接口为准；不要把预发布 CI 包当正式包。
...
| Windows x64 | 对外安装包 | 正式 `v1.0.0` NSIS `/S` 装到 `%LOCALAPPDATA%\Programs\@openworkdesktop\WodeAppX.exe`。CDP 标题 WodeAppX；DeepSeek 接通后打包 sidecar 真实「你好」回「你好！我是 WodeAppX…」（[32029053984](https://github.com/diankourenxia/wodeappx/actions/runs/32029053984)） | **unsigned**。安装目录仍叫 `@openworkdesktop` |
| Linux x64 | 对外安装包 | 腾讯云解包正式 `v1.0.0` AppImage + xvfb。空仓 First Mile 可开；写入 DeepSeek 后侧栏「本机 Key · 已配置」；打包 sidecar 真实 prompt「你好」回「你好！我是 WodeAppX…」（`linux-v100-engine-hello.json`） | xvfb 下 UI 作曲框有时不挂载，对话走打包引擎 HTTP，与 Mac L4 同类验收 |
