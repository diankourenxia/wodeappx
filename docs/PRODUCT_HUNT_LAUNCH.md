...
> 仓：https://github.com/diankourenxia/wodeappx（**公证 + 门禁过了再改 public**）  
> 站点：https://wodeapp.ai/wodeappx/  
> 品牌：开源对外 **WodeAppX**。主句与官网一致：**Customize the agent. Mix and match models.**

---

## 0. 上线闸门（未全绿不要改 public、不要点 Publish）

| 闸门 | 2026-08-17 实证 | 还差 |
|---|---|---|
| macOS `.app` 公证 | 本机 `release-oss/wodeappx-mac-arm64-1.0.0-oss.dmg` 内 `WodeAppX.app`：`spctl` = **accepted / Notarized Developer ID**；`stapler validate` 通过 | DMG 文件本身未签名（打开后跑里面的 app 即可） |
| `pnpm open-source:check` | PASS（1 warning：OpenWork 版本号与包版本不一致） | — |
| First Mile / 能力表单测 | 32 pass | — |
| L4 桌面活检 | 脚本已改认 `127.0.0.1:5174`；当天渲染进程 `body` 为空，判 FAIL | 开发窗恢复后再跑 `pnpm open-source:verify:l4` |
| i18n 门禁 | FAIL：`initLocale` 仍强制迁到中文，且 vendor 未吃到 overlay | 改成「用户选择 → 系统语言 → English」，再 `openwork:patch` |
| 独立仓导出 | GitHub 停在 8-14；缺 `TRADEMARK.md` / README Logo | `pnpm open-source:export` 后推私有仓 |
| 仓可见性 | 仍 private | 以上全绿后再改 public |
| 正式 `v*` Release | 只有预发布 `ci-packages-1.0.0` | 公证包挂到 `v1.0.0`（或下一个号） |
...
| **Website** | https://wodeapp.ai/wodeappx/ | — |
| **GitHub** | https://github.com/diankourenxia/wodeappx | public 之后再填 |
...
**Links**
- Site: https://wodeapp.ai/wodeappx/
