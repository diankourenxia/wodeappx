...
async function applyWebViteSplitPatch() {
  ...
  let content = await readFile(file, "utf8");
  if (content.includes("wodeapp web split chunks v3")) {
  ...
  if (content.includes(from)) {
    content = content.replace(from, to);
  } else if (content.includes(fromV1)) {
    // replace the whole build block from a previous v1 patch
