...
test("ensureStandaloneSelfEvolveSiblings adds runtime-server when packing wodeappx-only", async () => {
  const tree = mkdtempSync(path.join(tmpdir(), "wodeappx-pack-oss-"));
  mkdirSync(path.join(tree, "wodeappx", "scripts"), { recursive: true });
  writeFileSync(path.join(tree, "wodeappx", "package.json"), JSON.stringify({ name: "wodeappx" }));
  const first = await ensureStandaloneSelfEvolveSiblings(tree);
  assert.ok(first.added.includes("runtime-server/package.json"));
