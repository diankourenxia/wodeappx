...
/**
 * Standalone OSS checkout has no monorepo siblings. GHA pack then emits
 * `wodeapp/wodeappx/*` only, and packaged extract used to reject it.
 * Add the minimum siblings so `looksLikeMonorepoRoot` still passes.
 */
export async function ensureStandaloneSelfEvolveSiblings(treeRoot) {
  const added = [];
  try {
    await stat(path.join(treeRoot, "wodeappx"));
  } catch {
    return { added };
  }
  try {
    await stat(path.join(treeRoot, "runtime-server"));
  } catch {
    await mkdir(path.join(treeRoot, "runtime-server"), { recursive: true });
    const rel = "runtime-server/package.json";
    await writeFile(
      path.join(treeRoot, rel),
      `${JSON.stringify({
        name: "runtime-server",
        private: true,
        description: "OSS self-evolve sibling stub",
      }, null, 2)}\n`,
    );
    added.push(rel);
  }
  try {
    await stat(path.join(treeRoot, "AGENTS.md"));
  } catch {
    await writeFile(
      path.join(treeRoot, "AGENTS.md"),
      "# WodeAppX\n\nStandalone extract. Product source is under `wodeappx/`.\n",
    );
...
export async function packSelfEvolveSource(options = {}) {
  ...
  const siblings = await ensureStandaloneSelfEvolveSiblings(staged.treeRoot);
  if (siblings.added.length) {
    staged.files += siblings.added.length;
    for (const rel of siblings.added) {
