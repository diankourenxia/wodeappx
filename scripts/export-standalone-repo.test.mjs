...
import { validateRemote, validateGitArgs, validatePushOrphanWithoutForce, parseModeFlag } from "./export-standalone-repo.mjs";
...
test("parseModeFlag reads --mode=orphan equals form", () => {
  assert.equal(parseModeFlag(["--mode=orphan"]), "orphan");
});

test("parseModeFlag reads --mode orphan space form", () => {
  assert.equal(parseModeFlag(["--mode", "orphan"]), "orphan");
});

test("PUSH=1 + parsed --mode=orphan + no FORCE_EXPORT throws", () => {
  const mode = parseModeFlag(["--mode=orphan"]);
  assert.throws(
    () => validatePushOrphanWithoutForce(true, mode, false),
    /PUSH=1 with --mode=orphan requires FORCE_EXPORT=1/,
  );
});
